...
4. **절대 금지: 가짜 메커니즘 창작 및 변명 금지 (Zero Hallucination / Fact-Only)**:
   ...
   - **Speed Demon**: 요구 스탯은 **`Agility 25`**이며(Agility 40 + Strength 40 같은 가짜 요구치 엄금), 이동 속도 버프 중 타격 시 출혈(Bleed)을 유발하고 공속을 높여줍니다.
   - **Tough Love**: 요구 스탯은 **`Charisma 15~25`**이며, 매혹(Charm) 상태인 상대에게 가하는 피해량을 **+10% 증폭**시켜 주는 최고의 딜링 탤런트입니다.
   - **Ghost**: 'Q키로 구를 때 투명화 및 무적 시간(I-frame 1.2초)을 부여하는 수동 회피 탤런트'이며, 자동 회피나 추가 딜이 전혀 없습니다.
   - 사용자가 자신의 캐릭터 빌드 슬롯에 이미 보유하고 있는 탤런트(Speed Demon, Tough Love 등)를 '없다'고 부정하거나 헛소리하지 마세요.
